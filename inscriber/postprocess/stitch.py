...
def strip_running_headers_footers(
    ...
    for md in page_texts:
        ...
        tops.append(_norm_line(nonempty[0]) if nonempty else "")
        bottoms.append(_norm_line(nonempty[-1]) if nonempty else "")
    ...
    for md in page_texts:
        ...
        if idx_top is not None and _norm_line(lines[idx_top]) in headers:
            del lines[idx_top]
            removed += 1
        # Strip last non-empty line if it is a recurring footer.
        idx_bot = next((j for j in range(len(lines) - 1, -1, -1) if lines[j].strip()), None)
        if idx_bot is not None and _norm_line(lines[idx_bot]) in footers:
            del lines[idx_bot]
            removed += 1
        out.append("\n".join(lines).strip("\n"))
    if removed:
...
def stitch_pages(
    ...
    *,
    ...
    The ``#### Page N`` shape is what the splitter recognizes.
    """
    parts: list[str] = []
    for idx, (page_number, content) in enumerate(pages):
        if page_separators and idx > 0:
            parts.append("\n\n---\n\n")
        if page_numbers:
