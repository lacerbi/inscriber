...
Created: 2026-06-09
Status: APPROVED 2026-06-09 — build backend `hatchling` confirmed. Ready to execute
M0 on the user's explicit go-ahead; **not yet started** (no code written).
...
- `inscriber/config.py` — TOML load + 3-layer merge (**CLI > config file > default**),
  validation (fail-fast on missing model files/binary/unreadable PDF before any load).
...
- `inscriber/llama/server.py` — `LlamaServerManager`: ephemeral free-port probe,
  ...
  error with server-log tail), `contextmanager serve()`, atexit/signal teardown
  (DESIGN §5).
...
- `inscriber/ocr/base.py` — `Inferencer` Protocol + `HttpInferencer` (server) and
  ...
  `chat_template(path)`. (The `OcrBackend` ABC + `Region`/`OcrPageResult` dataclasses
  also live in this file but are added in **M1b**; M1a only needs the inference layer.)
- `--ocr-endpoint`/`--vlm-endpoint` escape hatch (DESIGN §5.1): when set, do **not**
  spawn — talk to the given URL. Wire the no-spawn branch here.
...
- `tests/fixtures/` — commit the recorded real outputs produced below.
...
2. **Grounding coordinate frame**: render a page with a figure at a known location,
   send `<|grounding|>Convert the document to markdown.` (`temperature:0`), read the
...
- `inscriber/postprocess/prompt.py` — **port verbatim** the figure-description prompt
  ...
  (`:41-51`) + `extractDescriptionFromTags` fallbacks (`:60-89`: no opening tag→treat
  whole response as desc + warn; opening but no closing→everything after opening).
...
- `inscriber/postprocess/splitter.py` — port `markdown-splitter.ts`: `extractTitle`
  ...
  faithful, don't "fix". Sources `markdown-splitter.ts`, `content-utils.ts`.
...
**Files to create**:
- `inscriber/input/resolver.py` — path (exists/readable/`%PDF` magic) or http(s) URL
  → `ResolvedInput(pdf_bytes, source, original_url, suggested_name)`; httpx download
  (follow redirects, timeout, descriptive UA, validate PDF bytes); `--offline` hard-
  errors on URL input.
...
- `inscriber/bibtex/semantic_scholar.py` — title→entry (API call per Correction #5),
  ...
  sentinel** on failure, mock-text assembly (Correction #2, standardized 4-line
  warning per DESIGN §12), prepend+fenced-code-block+`---` injection for
  `section ∈ {full,main,allparts}` via `bibtex.append_to_document`; standalone
  `paper.bib`; degrade on 429/offline/network-fail (never fail the run).
...
- [ ] All 7 transforms produce expected PDF URLs + filenames (fixtures); OpenReview
      keeps `?id=`.
- [ ] BibTeX: real-match path, mismatch warning, mock fallback, fenced prepend all
      match paper2llm byte-for-byte (modulo standardized warning).
...
- `LlamaServerManager` unit tests — launch-arg construction + `.exe` suffix logic with
  `Popen` mocked (no real spawn).
