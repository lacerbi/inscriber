...
Archived 2026-06-10 to `dev/plans/`; the per-milestone exit-gate checkboxes
below were ticked at archive time — each was verified at completion (evidence
in the Execution Progress tracker) but left unticked back then.
...
- [x] **M1b — OCR vertical slice**: ocr/registry, ocr/deepseek (REAL `LABEL[[bbox]]` parser + padded-square mapping), cache (OcrCache), serialize, test_deepseek_parser, test_cache — **DONE + verified on real hardware** (`dev/scripts/m1b_check.py`: sample_paper.pdf → clean markdown w/ placeholder; cache hit → no server). pipeline.run_ocr_pass built (shared by M2 run/ocr).
...
1. **`domain-handler-registry.ts` DOES exist** at `core/domain-handler-registry.ts`
   ...
   `domain-handlers/` _directory_ does only contain `base/generic/index` (design is
...
2. **Exact mock-BibTeX text** is richer than DESIGN §12's abbreviation. Real text in
   ...
   _(There are actually two mock variants in `content-utils.ts`: the no-result path
   ...
   mock per DESIGN §12; just don't be surprised by the second block.)_
...
5. **Semantic Scholar call**: `GET https://api.semanticscholar.org/graph/v1/paper/search`
   ...
   `[]`). DESIGN §12 wants graceful 429 degradation — _add_ a clean message + skip.
...
- `inscriber/config.py` — TOML load + 3-layer merge (**CLI > config file > default**),
  **two-layer validation** (review Fix 3): (a) _structural_ — enum membership, numeric
  ranges, types — runs always after merge, raises `ConfigError`; (b) _path-existence_ —
...
- [x] `inscriber --version` prints version (stdout).
- [x] `pip install -e ".[dev]"` succeeds in a fresh venv on Windows.
- [x] `pytest tests/test_config.py` green; precedence + validation covered.
- [x] `ruff check` clean.
...
committing the OCR backend. DESIGN §2.1, §2.2, §8.3 — _"Nothing else can be trusted
until this lands."_
...
- `tests/fixtures/` — (1) a **generated calibration PDF** (review Fix 4): a filled
  ...
  pixel box, so the coordinate-frame check below is _computable_, not eyeballed; (2) the
...
- [x] A real image round-trips (server path) OR mtmd-cli fallback proven, documented.
- [x] Coordinate frame determined with reproduced-box evidence; locked for M1b.
- [x] Fixtures committed; `dev/notes/2026-06-09-m1a-findings.md` written.
...
- [x] `test_deepseek_parser.py` green against real fixtures (tokens + locked frame).
- [x] Real PDF → per-page markdown with placeholders; cache hit on re-run (OCR server
...
- `inscriber/postprocess/prompt.py` — **port verbatim** the figure-description prompt
  ...
  warning. The "opening-but-no-closing → everything after the opening tag" half _is_ a
...
- Figure injection (DESIGN §10.2): replace `⟦INSCRIBER_FIG:{id}⟧` with blockquote;
  ...
  - desc), `placeholder`; single trailing `\n` per block. Port the _format_ from
...
at the end of M2 `describe` produces the _enhanced in-memory markdown_ (figures
...
- [x] `inscriber ocr paper.pdf -o out/` → inspectable bundle; `inscriber describe
    out/paper.inscriber-ocr` reuses crops; swapping `--vlm-model` re-describes only.
- [x] `test_bundle_roundtrip.py` green; schema gate + hand-edit survive.
- [x] Figure blockquotes match exact headers/spacing; `describe-and-keep` adds image ref.
...
- [x] Splitter passes the battery (esp. `A `-after-ack guard, out-of-order ack/appendix,
      page-marker boundary shift).
- [x] Stitch strips recurring headers/footers conservatively; de-hyphenation joins.
- [x] Output files written with correct names/encoding; no `paper.main` collision.
...
- `inscriber/bibtex/semantic_scholar.py` — title→entry (API call per Correction #5),
  ...
  block — and test against _that_, not paper2llm byte-parity. `bibtex.append_to_document`
...
- [x] All 7 transforms produce expected PDF URLs + filenames (fixtures); OpenReview
      keeps `?id=`.
- [x] BibTeX: real-match path, mismatch warning, and fenced prepend correct; mock
      fallback matches the **canonical inscriber mock fixture** (not paper2llm byte-parity).
- [x] `--offline` blocks URL input + BibTeX with clear messages; local servers
...
- [x] CI green on all 3 OSes (mocked).
- [x] Mocked end-to-end produces the full DESIGN §14 file set.
- [x] Built wheel installs and runs `inscriber --version` via pip and pipx.
- [x] Manual/integration doc validates one real PDF against real GGUFs (not in CI).
...
- **M1a empirical answers differ from design defaults** (coord frame = padded square,
  or server image path broken → mtmd-cli). _Mitigation_: M1a runs first and encapsulates
...
- **DeepSeek-OCR f16 repetition loops** (Q4_K_M banned; no n-gram penalty in llama.cpp).
  _Mitigation_: f16 + hard `max_tokens` cap + per-request wall-clock timeout + soft-fail
  on looping/truncated page; DRY/repeat-penalty flags as partial mitigation (DESIGN §2.2).
- **Port-fidelity drift** (regexes/strings subtly wrong). _Mitigation_: golden tests
  pin exact strings; each transform cites its paper2llm source; fixtures per domain.
- **Windows-specific breakage** (paths, `.exe`, `\r\n`, process teardown).
  _Mitigation_: cross-cutting conventions enforced from M0; CI matrix includes Windows.
